...
def process_answer(state: OnboardingState) -> OnboardingState:
    ...
    # Generate a contextual acknowledgment using memory + current answer
    from .vector_store import retrieve_memory
    past = retrieve_memory(state['session_id'], answer, k=2)
    memory_hint = ""
    if past:
        memory_hint = "Things they've already shared: " + " | ".join(past) + "\n"
    ack_prompt = (
        f"{memory_hint}"
        f"The user just answered a dating profile question.\n"
        f"Question: {question}\n"
        f"Answer: {answer}\n\n"
        "Write a single warm, natural sentence acknowledging what they said. "
        "If their answer connects to something they said before, briefly reflect that. "
        "Keep it conversational and genuine — like something you'd say on a first date. "
        "Do not ask a follow-up question. Output only the sentence, nothing else."
    )
    response = str(agent.llm.invoke(ack_prompt)).strip()
    if state['mode'] == 'voice':
        speak(response)
    else:
        print(response)
